...
<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-800 via-teal-700 to-emerald-900 text-white p-5 sm:p-8 sm:p-10 shadow-2xl shadow-emerald-900/20 border border-emerald-600/30">
        <div class="absolute -right-10 -bottom-10 opacity-10 text-white pointer-events-none hidden sm:block">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
            <div class="space-y-2 max-w-2xl">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 backdrop-blur-md text-emerald-200 text-[10px] sm:text-xs font-bold uppercase tracking-wider border border-emerald-400/30">
                    <i class="fa-solid fa-boxes-stacked"></i> Modul Pengajuan Multi-Alat Unit
                </span>
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-heading font-extrabold tracking-tight leading-tight">
    ...
    <!-- Form Pengajuan Alat (Multi-Item Cart - Full Width Card) -->
    <div class="glass-card rounded-3xl p-4 sm:p-7 sm:p-8 shadow-xl border border-slate-200/80 bg-white space-y-5 sm:space-y-6 relative z-20">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3.5 sm:pb-4">
            <div class="flex items-center gap-3 flex-1 min-w-0">
                <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-500 text-white flex items-center justify-center shadow-md shadow-emerald-600/20 flex-shrink-0">
                    <i class="fa-solid fa-cart-flatbed text-xs sm:text-sm"></i>
                </div>
                <div class="min-w-0">
                    <h3 class="font-heading font-extrabold text-base sm:text-lg text-slate-900 leading-tight truncate">
                        Formulir Pengajuan Multi-Alat Unit
                    </h3>
                    <p class="text-[11px] sm:text-xs text-slate-500 font-medium line-clamp-1 sm:line-clamp-none">Pilih satu atau beberapa alat dalam satu formulir resmi.</p>
                </div>
            </div>
            <span id="cartSummaryBadge" class="text-[11px] sm:text-xs font-extrabold px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 shadow-2xs whitespace-nowrap self-start sm:self-center">
                1 Jenis Alat
            </span>
        </div>

        <form action="<?= base_url('app/pengajuan-alat/store') ?>" method="POST" id="formPengajuanMulti" class="space-y-5 sm:space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
                <!-- Left: Multi-Item Rows Container (7 Cols) -->
                <div class="lg:col-span-7 space-y-3 sm:space-y-3.5">
                    <div class="flex items-center justify-between gap-2">
                        <label class="block text-[11px] sm:text-xs font-extrabold text-slate-700 uppercase tracking-wider flex items-center gap-1.5 truncate">
                            <i class="fa-solid fa-boxes-stacked text-emerald-600 flex-shrink-0"></i>
                            <span class="truncate">Daftar Alat Yang Diajukan <span class="text-rose-500">*</span></span>
                        </label>
                        <button type="button" onclick="addPengajuanRow()" class="text-[11px] sm:text-xs font-extrabold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl border border-emerald-200 transition flex items-center gap-1.5 shadow-2xs whitespace-nowrap flex-shrink-0">
                    ...
                    <div id="pengajuanItemsContainer" class="space-y-3">
                        <!-- Item Row 0 (Default) -->
                        <div class="pengajuan-item-card p-3.5 sm:p-4.5 rounded-2xl border border-slate-200/90 bg-slate-50/60 hover:border-emerald-300 transition space-y-3 relative group shadow-2xs" data-row-index="0">
                            <div class="flex items-center justify-between">
                                <span class="text-[10.5px] sm:text-[11px] font-extrabold text-slate-700 bg-white px-2.5 py-0.5 rounded-lg border border-slate-200/80 shadow-2xs item-number-label">
                            ...
                            <div class="space-y-2.5">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-2.5 sm:gap-3">
                                    <div class="flex-1">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Jumlah Diminta</label>
                                        <div class="relative">
                                    ...
                                    <div class="flex-shrink-0 flex sm:pb-1 items-center justify-start sm:justify-end">
                                        <div class="stock-info-badge inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-[10px] font-extrabold bg-slate-100 text-slate-500 border border-slate-200">
                ...
                <!-- Right: Alasan Keperluan, Petunjuk, & Tombol Submit (5 Cols) -->
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                ...
                                <span>Alasan Keperluan Pengajuan <span class="text-rose-500">*</span></span>
                            </label>
                            <textarea name="alasan_keperluan" rows="3.5" placeholder="Jelaskan kebutuhan permohonan alat ini (contoh: untuk persiapan bersih-bersih asrama baru / alat lama sudah rusak)..." required class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-semibold bg-slate-50/80 focus:bg-white focus:ring-2 focus:ring-emerald-500 transition shadow-2xs leading-relaxed"></textarea>
                        </div>

                        <div class="p-3.5 rounded-2xl bg-gradient-to-br from-emerald-50/80 to-teal-50/80 border border-emerald-200/80 text-emerald-950 space-y-1 shadow-2xs">
                            <div class="font-extrabold text-xs text-emerald-900 flex items-center gap-1.5">
                                <i class="fa-solid fa-circle-info text-emerald-600"></i> Catatan Proses
                            </div>
                            <p class="text-[11px] text-emerald-800/90 leading-relaxed">
                                Pengajuan Anda akan ditinjau langsung oleh Divisi Logistik & Gudang. Jumlah yang disetujui dapat disesuaikan dengan ketersediaan stok fisik.
                            </p>
                        </div>
                    </div>
    ...
    <!-- Tabel Riwayat Pengajuan Alat Unit Saya (Full Width Card) -->
    <div class="glass-card rounded-3xl p-4 sm:p-7 sm:p-8 shadow-xl border border-slate-200/80 bg-white space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-slate-100 pb-3.5 sm:pb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] sm:text-xs text-slate-500 font-medium">Daftar permohonan alat yang pernah diajukan oleh unit Anda.</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2.5 w-full sm:w-auto">
                <div class="relative flex-1 sm:w-64">
                    ...
                    <input type="text" id="searchMyPengajuanPageInput" onkeyup="filterMyPengajuanPageTable()" placeholder="Cari kode, alat, alasan..." class="w-full pl-9 pr-4 py-2 rounded-2xl border border-slate-200 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 focus:bg-white transition shadow-2xs">
                </div>
                <span class="text-[11px] sm:text-xs font-extrabold text-emerald-800 bg-emerald-50 px-3 py-2 rounded-2xl border border-emerald-200 whitespace-nowrap shadow-2xs">
            ...
            <table id="tableMyPengajuanPage" class="w-full text-left text-xs font-semibold">
                <thead class="bg-gradient-to-r from-emerald-800 to-teal-800 text-white font-heading font-extrabold uppercase text-[10px] tracking-wider">
                    <tr>
                        ...
                        <th width="16%" class="py-3.5 px-4">TANGGAL & KODE</th>
                        <th width="35%" class="py-3.5 px-4">DAFTAR PERALATAN</th>
                        <th width="25%" class="py-3.5 px-4">ALASAN & CATATAN ADMIN</th>
                        <th width="10%" class="py-3.5 px-4 text-center">STATUS</th>
                        <th width="10%" class="py-3.5 px-4 text-center">AKSI</th>
                    </tr>
                </thead>
                            ...
                            <tr class="my-pengajuan-page-row hover:bg-slate-50/90 transition-all">
                                ...
                                <td class="py-4 px-4 font-bold text-slate-600 whitespace-nowrap">
                                    ...
                                    <?php if (!empty($p['kode_pengajuan'])): ?>
                                        <div class="mt-1">
                                            <span class="px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 text-[10px] font-mono font-extrabold border border-emerald-200 shadow-2xs">
                                    ...
                                    <?php if (!empty($p['items']) && is_array($p['items'])): ?>
                                        <div class="space-y-1.5">
                                            <?php foreach ($p['items'] as $it): ?>
                                                <div class="p-2.5 rounded-xl bg-slate-50/90 border border-slate-200/80 flex items-center justify-between gap-2 shadow-2xs">
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <?php if (!empty($it['catatan_item'])): ?>
                                                            <div class="text-[10px] text-slate-500 italic truncate mt-0.5 pl-4"><?= esc($it['catatan_item']) ?></div>
                                                        <?php endif; ?>
                                                    </div>
                                                    <div class="text-right flex-shrink-0 flex items-center gap-1.5">
                                                        <span class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold bg-white border border-slate-200 text-slate-700 shadow-2xs" title="Jumlah Diminta">
                                                            Minta: <?= $it['jumlah_minta'] ?> <?= esc($it['satuan'] ?? 'Unit') ?>
                                                        </span>
                                                        <?php if ($p['status'] === 'Disetujui' || $p['status'] === 'Selesai' || $it['jumlah_setuju'] !== null): ?>
                                                            ...
                                                            <span class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold <?= (int)$it['jumlah_setuju'] > 0 ? ((int)$it['jumlah_setuju'] < (int)$it['jumlah_minta'] ? 'bg-amber-100 text-amber-900 border border-amber-300' : 'bg-emerald-100 text-emerald-900 border border-emerald-300') : 'bg-rose-100 text-rose-900 border border-rose-300' ?>" title="Jumlah Disetujui">
                                                                Disetujui: <?= (int)$it['jumlah_setuju'] ?> <?= esc($it['satuan'] ?? 'Unit') ?>
                                                            </span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                ...
                                <td class="py-4 px-4">
                                    ...
                                    <?php if (!empty($p['catatan_admin'])): ?>
                                        <div class="mt-2 p-2.5 rounded-xl bg-emerald-50/90 border border-emerald-200/90 text-emerald-900 text-[10.5px] font-semibold shadow-2xs">
                                            ...
                                            <div class="text-slate-700 font-medium mt-0.5 pl-4"><?= esc($p['catatan_admin']) ?></div>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="py-4 px-4 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="<?= base_url('cs/pengajuan/cetak/' . $p['id']) ?>" target="_blank" class="px-3 py-1.5 rounded-xl bg-teal-50 hover:bg-teal-100 text-teal-800 text-xs font-extrabold border border-teal-200/80 transition flex items-center gap-1.5 shadow-2xs" title="Cetak Bukti Serah Terima">
                                        ...
                                        <?php if ($p['status'] === 'Pending'): ?>
                                            <button type="button"
                                                    ...
                                                    class="px-3 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 hover:text-rose-800 text-xs font-extrabold border border-rose-200/80 transition flex items-center gap-1.5 shadow-2xs hover:shadow-sm"
...
<script>
    ...
    function closeAllRowAlatPickers() {
        document.querySelectorAll('.pengajuan-item-card').forEach(c => {
            c.style.zIndex = '';
        });
        document.querySelectorAll('.row-alat-picker-container').forEach(cnt => {
            cnt.style.zIndex = '';
        });
    ...
    function addPengajuanRow() {
        ...
        newCard.className = 'pengajuan-item-card p-3.5 sm:p-4.5 rounded-2xl border border-slate-200/90 bg-slate-50/60 hover:border-emerald-300 transition space-y-3 relative group shadow-2xs animate-fade-in';
